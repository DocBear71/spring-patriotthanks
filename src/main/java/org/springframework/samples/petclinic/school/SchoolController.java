 ...
 *
 * <p>
 * School detail pages can be accessed by numeric ID (e.g., {@code /schools/1}) or
 * by a slug derived from the school's domain (e.g., {@code /schools/kirkwood}).
 * When accessed by slug, the controller also loads the authenticated user's profile
 * data to display as a "Player Card" sidebar.
 * </p>
 *
 * @author Edward
 */
...
public class SchoolController {
	 ...
	 * @param userRepository   the repository for looking up the authenticated user's profile
	 */
	...
	/**
	 * Processes the school creation form submission. Validates input and saves
	 * the new school to the database.
	 *
	 ...
	 * @return a redirect to the school list on success, or the form view if validation fails
	 */
	 ...
	 * @return a {@link ModelAndView} containing the school object and the detail view name
	 ...
	 * their profile data is loaded and added to the model as {@code currentUser} so
	 * it can be displayed in the "Player Card" sidebar. The user's phone number is
	 * formatted from a raw 10-digit string to {@code (XXX) XXX-XXXX} for display.
	 * </p>
	 *
